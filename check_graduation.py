...
# データ読み込み
required = load_json("required_courses.json")
elective = load_json("elective_courses.json")
general = load_json("general_courses.json")
fusion = load_json("fusion_courses.json")
clasify = load_json("clasify.json")
user = load_json("my_courses.json")
...
category["専門科目（自由）"] = sum(c["credits"] for c in user if c["code"] not in known)
...
合算37 = 合算22 + 導入 + 選択 + category["専門科目（自由）"]
